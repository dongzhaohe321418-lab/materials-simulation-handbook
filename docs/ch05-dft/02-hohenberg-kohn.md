...
!!! example "Janak in action: ionisation potential of hydrogen"
    For the hydrogen atom with exact $E_{xc}$, sweeping the occupation $f$ of the 1s orbital from 1 to 0 traces out the curve $E(f) = E(1) - I\cdot(1-f)$ exactly, where $I=0.5\;\text{Ha}$ is the ionisation potential. The slope at any $f\in(0,1)$ equals $-I$. With approximate functionals like LDA, the curve is concave-up — $E(f)$ bows below the straight line — and the slope at $f=1$ underestimates $I$ by typically 5 eV (the LDA HOMO of H is $\approx -7\;\text{eV}$ versus experiment $-13.6\;\text{eV}$).
